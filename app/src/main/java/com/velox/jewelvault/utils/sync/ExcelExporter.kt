...
class ExcelExporter(private val context: Context) {
    ...
    private suspend fun exportStoreEntity(database: AppDatabase, workbook: Workbook, headerStyle: CellStyle) {
        ...
        val headers = listOf(
            ...
            "jurisdiction", "registrationNo", "gstinNo", "panNo", "image", "invoiceNo", "upiId", "lastUpdated"
        )
        ...
        stores.forEachIndexed { rowIndex, store ->
            ...
            row.createCell(7).setCellValue(store.jurisdiction)
            row.createCell(8).setCellValue(store.registrationNo)
            row.createCell(9).setCellValue(store.gstinNo)
            row.createCell(10).setCellValue(store.panNo)
            row.createCell(11).setCellValue(store.image)
            row.createCell(12).setCellValue(store.invoiceNo.toDouble())
            row.createCell(13).setCellValue(store.upiId)
            row.createCell(14).setCellValue(store.lastUpdated.toDouble())
        }
    ...
    private suspend fun exportOrderEntity(database: AppDatabase, workbook: Workbook, headerStyle: CellStyle) {
        ...
        val headers = listOf("orderId", "customerMobile", "storeId", "userId", "orderDate", "totalAmount", "totalTax", "totalCharge", "discount", "invoiceNo", "note")
        ...
        orders.forEachIndexed { rowIndex, order ->
            ...
            row.createCell(9).setCellValue(order.invoiceNo)
            row.createCell(10).setCellValue(order.note ?: "")
        }
    ...
    private suspend fun exportOrderItemEntity(database: AppDatabase, workbook: Workbook, headerStyle: CellStyle) {
        ...
        val headers = listOf(
            "orderItemId", "orderId", "orderDate", "itemId", "customerMobile", "invoiceNo", "catId", "catName", "itemAddName", "subCatId", "subCatName", "entryType", "quantity", "gsWt", "ntWt", "fnWt", "fnMetalPrice", "purity", "crgType", "crg", "compDes", "compCrg", "cgst", "sgst", "igst", "huid", "addDesKey", "addDesValue", "price", "charge", "tax", "sellerFirmId", "purchaseOrderId", "purchaseItemId"
        )
        ...
        orderItems.forEachIndexed { rowIndex, orderItem ->
            ...
            row.createCell(5).setCellValue(orderItem.invoiceNo)
            row.createCell(6).setCellValue(orderItem.catId)
            row.createCell(7).setCellValue(orderItem.catName)
            row.createCell(8).setCellValue(orderItem.itemAddName)
            row.createCell(9).setCellValue(orderItem.subCatId)
            row.createCell(10).setCellValue(orderItem.subCatName)
            row.createCell(11).setCellValue(orderItem.entryType)
            row.createCell(12).setCellValue(orderItem.quantity.toDouble())
            row.createCell(13).setCellValue(orderItem.gsWt)
            row.createCell(14).setCellValue(orderItem.ntWt)
            row.createCell(15).setCellValue(orderItem.fnWt)
            row.createCell(16).setCellValue(orderItem.fnMetalPrice)
            row.createCell(17).setCellValue(orderItem.purity)
            row.createCell(18).setCellValue(orderItem.crgType)
            row.createCell(19).setCellValue(orderItem.crg)
            row.createCell(20).setCellValue(orderItem.othCrgDes)
            row.createCell(21).setCellValue(orderItem.othCrg)
            row.createCell(22).setCellValue(orderItem.cgst)
            row.createCell(23).setCellValue(orderItem.sgst)
            row.createCell(24).setCellValue(orderItem.igst)
            row.createCell(25).setCellValue(orderItem.huid)
            row.createCell(26).setCellValue(orderItem.addDesKey)
            row.createCell(27).setCellValue(orderItem.addDesValue)
            row.createCell(28).setCellValue(orderItem.price)
            row.createCell(29).setCellValue(orderItem.charge)
            row.createCell(30).setCellValue(orderItem.tax)
            row.createCell(31).setCellValue(orderItem.sellerFirmId)
            row.createCell(32).setCellValue(orderItem.purchaseOrderId)
            row.createCell(33).setCellValue(orderItem.purchaseItemId)
        }
