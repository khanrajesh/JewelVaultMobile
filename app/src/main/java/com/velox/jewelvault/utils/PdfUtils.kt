...
fun generateDraftInvoicePdf(
    ...
    canvas.apply {
        ...
        drawText("Name: ${data.customerInfo.name} (${data.customerInfo.mobileNo})", nameTextStartX, nameText1Y, tp16Bold )
        ...
        val columns = listOf(
            "Sr", "Ref No.", "Product Description", "Qty/Set", "Gross.Wt\nin GMS", "Net.Wt\nin GMS", "Rate/Gm", "Making\nAmount", "Purity %", "E.G", "Total Amount"
        ...
        for (i in columns.indices) {
            ...
            if (lines.size == 2) {
                ...
                drawText(lines[1], xCenter - text2Width / 2, headingY + headingLineSpacing, tp16Bold)
            } else {
                val textWidth = tp16Bold.measureText(heading)
                drawText(heading, xCenter - textWidth / 2, headingY + headingLineSpacing / 2, tp16Bold)
            }
        }
...
//        )
        for (rowIndex in itemRows.indices) {
            ...
            for (colIndex in row.indices) {
                ...
                if (colIndex == columns.size - 1) {
                    drawText(value, minOf(xCenter - textWidth / 2, itemBoxRight - s8f - textWidth), y - s3f, tp16)
                } else {
        ...
        drawLine(goldLabelX, goldLabelY + s2f, goldLabelX + goldLabelWidth, goldLabelY + s2f, innerBoxPaint)

        for (i in headings.indices) {
            val heading = headings[i]
            val textWidth = tp16Bold.measureText(heading)
            val x = s5f + i * headingColumnWidth + (headingColumnWidth - textWidth) / 2
            val y = goldLabelY+s10f+s2f // vertical centering in the box
            drawText(heading, x, y, tp16Bold)
        }
        ...
        for (i in headings.indices) {
            ...
            val textWidth = tp16Bold.measureText(heading)
            val x = s5f + i * headingColumnWidth + (headingColumnWidth - textWidth) / 2
            val y = silverLabelY+s10f+s2f // vertical centering in the box
            drawText(heading, x, y, tp16Bold)
        }
        ...
        drawText("Amount in words: ${data.paymentSummary.amountInWords}", startX + s2f, amountInWordsY, tp16)
        ...
        drawLine(startX, declarationY + s2f, startX + declarationTitleWidth, declarationY + s2f, innerBoxPaint)
        // Declaration points
        ...
        drawLine(currentRateX, currentRateY + s2f, currentRateX + currentRateWidth, currentRateY + s2f, innerBoxPaint)
        ...
        drawText(data.silverRate, metalBoxRight+s2f+s50f,metalBoxTop+s9f+s9f+s9f +s2f, tp14)

        drawText("Payment Details", metalBoxRight+s2f,metalBoxTop+s9f+s9f+s9f+s9f + s2f, tp16)
        ...
        drawLine(paymentDetailsX, paymentDetailsY + s2f +s2f, paymentDetailsX + paymentDetailsWidth, paymentDetailsY + s2f, innerBoxPaint)
        ...
        drawText("${data.paymentReceived.cashAmount1}", metalBoxRight+s2f+s50f,cashText1Y, tp14)
...
//        canvas.drawText(thankYouText, centerX, thankYouY, thankYouPaint)
        ...
        data.customerSignature?.let { customerSign ->
            ...
            val customerSrcRect = android.graphics.Rect(0, 0, customerBitmap.width, customerBitmap.height)
...
fun generateTaxInvoicePdf(
    ...
    // Header section
    canvas.drawRect(margin, headerStartY, pageWidth - margin, headerStartY + headerHeight, borderPaint)
    canvas.drawRect(margin, headerStartY, margin + companySectionWidth, headerStartY + headerHeight, borderPaint)
    canvas.drawRect(margin + companySectionWidth, headerStartY, pageWidth - margin, headerStartY + headerHeight, borderPaint)
    
    // Customer section
    canvas.drawRect(margin, customerStartY, pageWidth - margin, customerStartY + customerHeight, borderPaint)
    ...
    canvas.drawRect(summaryX, summaryStartY, pageWidth - margin, summaryStartY + summaryHeight, borderPaint)
    
    // Terms section
    canvas.drawRect(margin, termsStartY, margin + termsWidth, termsStartY + termsHeight, borderPaint)
    ...
    y += gapY
    paint.textSize = 9f
    canvas.drawText(store.address ?: "Old medical road, Malkangiri, Odisha - 764048", startX, y, paint)
    y += gapY
    ...
    y += gapY
    canvas.drawText("Invoice Date: ${SimpleDateFormat("dd-MMM-yyyy", Locale.getDefault()).format(Date())}", rightX, y, paint)
    y += gapY + 10f
    ...
    paint.textSize = 7f
    colX = margin + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4] + colWidths[5] + 5f
    ...
    paint.textSize = 8f
    items.forEachIndexed { index, item ->
        val itemName = "${item.catName ?: ""} ${item.subCatName ?: ""} ${item.itemAddName ?: "Item"}".trim()
    ...
    y += gapY
    paint.textSize = 9f
    canvas.drawText("Taxable Amount: ${String.format("%.2f", totalTaxableValue)}", summaryX + 5f, y, paint)
    y += gapY
    ...
    y += gapY
    headerPaint.textSize = 11f
    canvas.drawText("Total Amount After Tax: ₹${String.format("%.2f", grandTotal)} (E & O.E.)", summaryX + 5f, y, headerPaint)
    y += gapY + 20f
    ...
    y = certStartY + 15f
    paint.textSize = 9f
    canvas.drawText("Certified that the particulars given above are true and correct.", startX, y, paint)
    y += gapY + 10f
...
fun generateEstimatePdf(
    ...
    canvas.drawText("Name : ${store.proprietor ?: "Ranjan khan"}", topRightX, contactY, regularPaint)
    contactY += 12f
    ...
    currentY = maxOf(currentY, contactY) + 15f // Ensure currentY is below both left and right top sections
    ...
    // GSTIN Box
    canvas.drawRect(margin, gstinBoxStartY, margin + gstinBoxWidth, gstinBoxStartY + sectionHeight, thickBorderPaint)
    boldPaint.textSize = 9f
    regularPaint.textSize = 9f
    canvas.drawText("GSTIN : ", margin + 5f, gstinBoxStartY + 17f, boldPaint) // Label bold
    canvas.drawText(store.gstinNo ?: "21APEPK7976C1ZZ", margin + 5f + boldPaint.measureText("GSTIN : "), gstinBoxStartY + 17f, regularPaint)
    ...
    regularPaint.textAlign = Paint.Align.RIGHT
    canvas.drawText(customerCopyText, pageWidth - margin, gstinBoxStartY - 8f , regularPaint) // Position above the estimate details
    ...
    canvas.drawText(":", estimateDetailLabelX + boldPaint.measureText("Estimate No.") + 2f, gstinBoxStartY + 17f, boldPaint)
    canvas.drawText(estimateNumber, estimateDetailValueX , gstinBoxStartY + 17f, regularPaint)

    val estimateDateLabelX = estimateDetailsX + (contentWidth * 0.45f / 2) + 10f // Position Date more to the right
    val estimateDateValueX = estimateDateLabelX + 55f
    canvas.drawText("Est. Date", estimateDateLabelX, gstinBoxStartY + 17f, boldPaint)
    canvas.drawText(":", estimateDateLabelX + boldPaint.measureText("Est. Date") + 2f, gstinBoxStartY + 17f, boldPaint)
    ...
    canvas.drawRect(margin, customerBoxStartY, pageWidth - margin, customerBoxStartY + customerBoxHeight, thickBorderPaint)
    ...
    customerDetailY = customerBoxStartY + 12f // Reset Y for second column of customer details (if any)
    ...
    canvas.drawText(customer.address ?: "N/A", customerCol1ValueX, customerDetailY + 11f, regularPaint)
    ...
    canvas.drawRect(margin, tableStartY, pageWidth - margin, tableStartY + tableHeaderHeight, tableHeaderBgPaint)
    ...
    canvas.drawText("Name of Product / Service", currentX + colWidths[1] / 2, headerTextY + 5f, tableHeaderPaint)
    ...
    canvas.drawText("CGST", cgstBaseX + (colWidths[6] + colWidths[7]) / 2, headerTextY, tableHeaderPaint)
    canvas.drawText("%", cgstBaseX + colWidths[6] / 2, subHeaderTextY, tableHeaderPaint)
    canvas.drawText("Amount", cgstBaseX + colWidths[6] + colWidths[7] / 2, subHeaderTextY, tableHeaderPaint)
    currentX += colWidths[6] + colWidths[7]

    // SGST (with sub-headers)
    val sgstBaseX = currentX
    canvas.drawText("SGST", sgstBaseX + (colWidths[8] + colWidths[9]) / 2, headerTextY, tableHeaderPaint)
    canvas.drawText("%", sgstBaseX + colWidths[8] / 2, subHeaderTextY, tableHeaderPaint)
    canvas.drawText("Amount", sgstBaseX + colWidths[8] + colWidths[9] / 2, subHeaderTextY, tableHeaderPaint)
    ...
    items.forEachIndexed { index, item ->
        ...
        canvas.drawText(item.itemAddName.take(25), currentX + 3f, textY, tableCellPaint) // Truncate if too long
        currentX += colWidths[1]
        // HSN/SAC
        tableCellPaint.textAlign = Paint.Align.CENTER
        ...
        currentX += colWidths[2]
        // Qty
        canvas.drawText(String.format("%.2f Gms", item.gsWt), currentX + colWidths[3] / 2, textY, tableCellRightAlignPaint) // Assuming gWt is quantity
        currentX += colWidths[3]
        // Rate
        canvas.drawText(String.format("%.2f", item.fnMetalPrice), currentX + colWidths[4] -3f, textY, tableCellRightAlignPaint)
        currentX += colWidths[4]
        // Taxable Value
        val taxableValue = item.price // Assuming item.price is taxable value before tax
        canvas.drawText(String.format("%.2f", taxableValue), currentX + colWidths[5] -3f, textY, tableCellRightAlignPaint)
        currentX += colWidths[5]
        ...
        canvas.drawText(String.format("%.2f", cgstRate), currentX + colWidths[6] -3f, textY, tableCellRightAlignPaint)
        currentX += colWidths[6]
        canvas.drawText(String.format("%.2f", cgstAmount), currentX + colWidths[7] -3f, textY, tableCellRightAlignPaint)
        ...
        canvas.drawText(String.format("%.2f", sgstRate), currentX + colWidths[8] -3f, textY, tableCellRightAlignPaint)
        currentX += colWidths[8]
        canvas.drawText(String.format("%.2f", sgstAmount), currentX + colWidths[9] -3f, textY, tableCellRightAlignPaint)
        currentX += colWidths[9]
        // Total
        val itemTotal = taxableValue + cgstAmount + sgstAmount // item.price already includes this usually
        canvas.drawText(String.format("%.2f", item.price), currentX + colWidths[10] -3f, textY, tableCellRightAlignPaint)
    ...
    // Table Border
    canvas.drawRect(margin, tableStartY, pageWidth - margin, tableRowY + tableBottomPadding, thickBorderPaint) // Outer table border
    canvas.drawLine(margin, tableStartY + tableHeaderHeight, pageWidth - margin, tableStartY + tableHeaderHeight, thickBorderPaint) // Header bottom line
    // Vertical Column Lines
    currentX = margin
    colWidths.forEach { width ->
        canvas.drawLine(currentX, tableStartY, currentX, tableRowY + tableBottomPadding, thickBorderPaint)
        currentX += width
    }
    canvas.drawLine(pageWidth - margin, tableStartY, pageWidth-margin, tableRowY + tableBottomPadding, thickBorderPaint) // Ensure last vertical line
    ...
    canvas.drawRect(margin, totalRowY, pageWidth - margin, totalRowY + totalRowHeight, tableHeaderBgPaint) // Background for total row
    canvas.drawLine(margin, totalRowY + totalRowHeight, pageWidth - margin, totalRowY + totalRowHeight, thickBorderPaint)
    ...
    val totalTaxableValue = items.sumOf { it.price - (it.cgst + it.sgst) } // Recalculate if price includes tax
    ...
    canvas.drawText(String.format("%.2f Gms", totalQty), currentX - colWidths[3]/2, totalTextY, tableCellRightAlignPaint) // Total Qty

    currentX += colWidths[4] // Skip Rate column
    currentX += colWidths[5]
    canvas.drawText(String.format("%.2f", totalTaxableValue), currentX -3f, totalTextY, tableCellRightAlignPaint)

    currentX += colWidths[6] // Skip CGST %
    currentX += colWidths[7]
    canvas.drawText(String.format("%.2f", totalCGST), currentX -3f, totalTextY, tableCellRightAlignPaint)

    currentX += colWidths[8] // Skip SGST %
    currentX += colWidths[9]
    canvas.drawText(String.format("%.2f", totalSGST), currentX -3f, totalTextY, tableCellRightAlignPaint)

    currentX += colWidths[10]
    canvas.drawText(String.format("%.2f", grandTotal), currentX -3f, totalTextY, tableCellRightAlignPaint)
    ...
    val words = try { numberToWords(grandTotal.toInt()) + " ONLY" } catch (e: Exception) { "Error in words" }
    ...
    canvas.drawText(String.format("₹%.2f", grandTotal - discount), summaryValueX, summaryY, boldPaint)
    summaryY += summaryLineHeight
    boldPaint.textAlign = Paint.Align.LEFT
    ...
    val contentValues = ContentValues().apply {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
            put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/JewelVault/Estimates")
        } else {
    ...
    try {
        ...
        uri?.let { resolver.delete(it, null, null) } // Clean up if URI was created but writing failed
        uri = null
    } finally {
        pdfDocument.close()
        if (uri == null) {
