...
interface CustomerKhataBookDao {
    ...
    @Query("SELECT * FROM ${TableNames.CUSTOMER_KHATA_BOOK} WHERE customerMobile = :customerMobile AND status = 'active' ORDER BY startDate DESC")
    ...
    @Query("SELECT * FROM ${TableNames.CUSTOMER_KHATA_BOOK} WHERE userId = :userId AND storeId = :storeId AND status = 'active' ORDER BY startDate DESC")
    suspend fun getActiveKhataBooks(userId: String, storeId: String): List<CustomerKhataBookEntity>
    
    @Query("SELECT * FROM ${TableNames.CUSTOMER_KHATA_BOOK} WHERE userId = :userId AND storeId = :storeId AND status = 'completed'")
    suspend fun getMaturedKhataBooks(userId: String, storeId: String): List<CustomerKhataBookEntity>
    
    @Query("SELECT * FROM ${TableNames.CUSTOMER_KHATA_BOOK} WHERE userId = :userId AND storeId = :storeId")
    fun getAllKhataBooks(userId: String, storeId: String): Flow<List<CustomerKhataBookEntity>>
    
    @Query("SELECT * FROM ${TableNames.CUSTOMER_KHATA_BOOK} WHERE khataBookId = :khataBookId")
    suspend fun getKhataBookById(khataBookId: String): CustomerKhataBookEntity?
    
    // Get khata books by plan name
    @Query("SELECT * FROM ${TableNames.CUSTOMER_KHATA_BOOK} WHERE customerMobile = :customerMobile AND planName = :planName AND status = 'active'")
    ...
    @Query("SELECT * FROM ${TableNames.CUSTOMER_TRANSACTION} WHERE khataBookId = :khataBookId AND transactionType = 'khata_payment'")
    suspend fun getPaidMonths(khataBookId: String): List<CustomerTransactionEntity>
    
    @Query("SELECT * FROM ${TableNames.CUSTOMER_TRANSACTION} WHERE customerMobile = :customerMobile AND transactionType = 'khata_payment' ORDER BY transactionDate DESC")
    fun getCustomerKhataPayments(customerMobile: String): Flow<List<CustomerTransactionEntity>>
    
    @Query("SELECT SUM(amount) FROM ${TableNames.CUSTOMER_TRANSACTION} WHERE khataBookId = :khataBookId AND transactionType = 'khata_payment'")
    suspend fun getTotalPaidAmount(khataBookId: String): Double?
    
    @Query("SELECT COUNT(*) FROM ${TableNames.CUSTOMER_TRANSACTION} WHERE khataBookId = :khataBookId AND transactionType = 'khata_payment'")
    suspend fun getPaidMonthsCount(khataBookId: String): Int
    ...
    """)
    suspend fun getKhataBookSummaries(userId: String, storeId: String): List<KhataBookSummary>
    
    @Query("SELECT * FROM ${TableNames.CUSTOMER_KHATA_BOOK}")
