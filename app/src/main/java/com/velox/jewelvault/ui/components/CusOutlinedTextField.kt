...
fun CusOutlinedTextField(
    ...
    allowEditOnDropdown = allowEditOnDropdown,
...
@Composable
fun RowScope.CusOutlinedTextField(
    state: InputFieldState,
    modifier: Modifier = Modifier,
    rowWeight: Float = 0f,
    ...
    state = state,
    modifier = if (rowWeight > 0f) modifier.weight(rowWeight) else modifier,
    onTextChange = onTextChange,
    placeholderText = placeholderText,
    dropdownItems = dropdownItems,
    onDropdownItemSelected = onDropdownItemSelected,
...
@Composable
fun ColumnScope.CusOutlinedTextField(
    state: InputFieldState,
    modifier: Modifier = Modifier,
    rowWeight: Float = 0f,
    columnWeight: Float = 0f,
    ...
    state = state,
    modifier = if (columnWeight > 0f) modifier.fillMaxWidth(columnWeight) else modifier,
    onTextChange = onTextChange,
    placeholderText = placeholderText,
    dropdownItems = dropdownItems,
    onDropdownItemSelected = onDropdownItemSelected,
...
private fun CusOutlinedTextFieldInternal(
    ...
    val enhancedKeyboardActions = remember(keyboardActions, nextFocusRequester) {
        if (nextFocusRequester != null) {
            KeyboardActions(
                onNext = {
                    // Simple focus request with error handling
                    try {
                        nextFocusRequester.requestFocus()
                    } catch (e: Exception) {
                        // If focus request fails, just continue with default behavior
                        // This can happen if the target field is not yet composed
                    }
                },
                onDone = {
            ...
            // If no next focus requester, use default keyboard actions
            keyboardActions
        }
    }
