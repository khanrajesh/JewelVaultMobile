...
private fun PurchaseDetails(viewModel: OrderAndReportViewModel) {
    ...
    val purchaseItems = viewModel.purchase.mapIndexed { index, item ->
        ...
        listOf(
            ...
            "${item.order.billNo} ",
            r,
            "Gold: ${CalculationUtils.calculateTotalExchangeWeightByMetal(item.exchanges, "Gold")}0 gm" +
                    "\nSilver: ${CalculationUtils.calculateTotalExchangeWeightByMetal(item.exchanges, "Silver")}0 gm ",
        )
    }
...
private fun OrderDetail(viewModel: OrderAndReportViewModel) {
    ...
    val orderItems = viewModel.orderList.mapIndexed { index, item ->
        ...
        listOf(
            ...
            "₹${CalculationUtils.totalPrice(item.totalAmount, item.totalCharge, 0.0,item.totalTax).to2FString()}"
        )
    }
